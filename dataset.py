...
class SteelDataset(Dataset):
    ...
    def __str__(self):
        ...
        pos2 = ((self.df['Class']==2) & (self.df['Label']==2)).sum()
        pos3 = ((self.df['Class']==3) & (self.df['Label']==3)).sum()
        pos4 = ((self.df['Class']==4) & (self.df['Label']==4)).sum()
...
def run_check_train_dataset():
    ...
    for n in range(0,len(dataset)):
        i = n # i = np.randome.choice(len(dataset))

        image, mask, infor = dataset[i]
        overlay = np.vstack([m for m in mask])

        #---
        print('%5d: %s'%(i, infor.image_id))
...
def run_check_data_loader():
    ...
    for t, (input, truth, infor) in enumerate(loader):
        ...
        if 1:
            ...
            # input = input.data.cpu().numpy()# run on cuda env
            input = (input*255).astype(np.uint8)
            input = input.transpose(0,2,3,1)
            #input = 255-(input*255).astype(np.uint8)

            # truth = truth.data.cpu().numpy()# run on cuda env
            for b in range(batch_size):
